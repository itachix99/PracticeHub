...
export function ExamHeader({ title, remaining, remainingSec, warningSec = 300 }: Props) {
  ...
  return (
    <header className="sticky top-0 z-10 flex h-14 items-center justify-between border-b bg-card px-4">
      ...
      <div className="flex items-center gap-3">
        <Badge variant={isCritical ? "destructive" : isWarning ? "secondary" : "outline"} className="gap-1.5 py-1.5">
          {isCritical && <AlertTriangle className="size-3.5" />}
          <Clock className="size-3.5" />
          <span className="font-mono text-sm font-bold">{remaining}</span>
        </Badge>
        <span className="hidden text-xs text-muted-foreground sm:inline">Time Left</span>
      </div>
    </header>
  );
}
