...
async function main() {
  ...
  if (existing) {
    ...
    for (const v of existing.versions) {
      await prisma.question.deleteMany({
        where: { sectionId: { in: v.sections.map((s) => s.id) } },
      });
  ...
  const admin = await prisma.user.findUnique({
    where: { email: "admin@practicehub.local" },
  });
  ...
  for (let batch = 0; batch < 10; batch++) {
    ...
    await prisma.$transaction(async (tx) => {
      for (let i = 0; i < batchSize; i++) {
        ...
        await tx.answer.create({
          data: {
            questionId: q.id,
            correctOptionId: (await tx.questionOption.findFirst({
              where: { questionId: q.id, label: "A" },
            }))!.id,
          },
        });
        created++;
    ...
    console.log(
      `[perf] batch ${batch + 1}/10  ${batchElapsed.toFixed(0)}ms  (${created} created)`
    );
  }

  const createElapsed = performance.now() - startCreate;
  console.log(
    `[perf] created exam ${exam.slug} with ${created} questions in ${createElapsed.toFixed(0)}ms`
  );
  ...
  const qCount =
    fetched?.currentVersion?.sections.reduce(
      (a, s) => a + s.questions.length,
      0
    ) ?? 0;
  console.log(
    `[perf] fetch exam with sections+questions: ${fetchElapsed.toFixed(2)}ms  (${qCount} questions)`
  );
  ...
  console.log(
    `[perf] report query (status OPEN): ${reportElapsed.toFixed(2)}ms`
  );
