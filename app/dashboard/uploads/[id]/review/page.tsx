...
export default async function ReviewPage({ params }: Props) {
  ...
  const upload = await prisma.paperUpload.findUnique({
    where: { id },
    include: {
      draftQuestions: { orderBy: { order: "asc" } },
  ...
  if (drafts.length === 0) {
    ...
    if (result?.structured) {
      try {
        const structuredRaw = result.structured as unknown;
        const structured = (
          typeof structuredRaw === "string"
            ? JSON.parse(structuredRaw as string)
            : structuredRaw
        ) as {
          questions?: Array<{
            text: string;
        ...
        if (qs.length > 0) {
          ...
          const rawVal = result.raw as unknown;
          const raw = rawVal
            ? ((typeof rawVal === "string"
                ? JSON.parse(rawVal as string)
                : rawVal) as { aiNeedsReview?: boolean })
            : {};
          ...
          drafts = await prisma.draftQuestion.findMany({
            where: { paperUploadId: id },
            orderBy: { order: "asc" },
          });
        }
      } catch {}
  ...
  const initialDrafts = drafts.map((d) => ({
    ...
    text: d.text,
    type: d.type,
    options: (typeof d.options === "string"
      ? JSON.parse(d.options as string)
      : d.options) as Array<{ label: string; text: string }>,
    ...
    <div className="container mx-auto max-w-4xl px-4 py-8 sm:px-6">
      <div className="mb-6 flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground text-sm">
            Upload {upload.fileName} • {upload.status} • {initialDrafts.length}{" "}
            drafts
          </p>
        </div>
      ...
      {initialDrafts.length === 0 ? (
        <div className="rounded-md border p-8 text-center">
          <p className="text-muted-foreground text-sm">
            No draft questions found. Ensure extraction completed
            (REVIEW_REQUIRED) and has questions.
          </p>
          <p className="text-muted-foreground mt-2 text-xs">
            Job status: {upload.jobs[0]?.status ?? "unknown"} • Try re-upload or
            check logs.
          </p>
        </div>
      ) : (
