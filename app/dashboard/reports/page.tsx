...
export default async function ReportsPage({
  ...
  const filterStatus =
    status && ["OPEN", "RESOLVED", "REJECTED"].includes(status)
      ? (status as "OPEN" | "RESOLVED" | "REJECTED")
      : undefined;
  ...
  return (
    <div className="container mx-auto max-w-7xl px-4 py-8 sm:px-6">
      <div className="mb-6 flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground text-sm">
            Review community reports • {reports.length} shown
          </p>
        </div>
      ...
      <div className="mb-4 flex flex-wrap gap-2">
        ...
        <Link href="/dashboard/reports?status=OPEN">
          <Button
        ...
        <Link href="/dashboard/reports?status=RESOLVED">
          <Button
        ...
        <Link href="/dashboard/reports?status=REJECTED">
          <Button
