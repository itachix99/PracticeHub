...
import { getDraftsForUpload, ensureDraftsExist } from "@/lib/services/draft.service";
...
export async function GET(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  return NextResponse.json({ drafts: drafts.map(d => ({ ...d, options: JSON.parse(d.options) })), upload: uploadWithJobs });
}
...
export async function PATCH(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!body || !Array.isArray(body.updates)) return NextResponse.json({ error: "Invalid body: expected { updates: [...] }" }, { status: 400 });
  ...
  for (const u of body.updates) {
    ...
    if (!parsed.success) return NextResponse.json({ error: `Invalid update for ${u.id}`, details: parsed.error.flatten() }, { status: 400 });
    // Ensure draft belongs to this upload
    const draft = await prismaClient.draftQuestion.findUnique({ where: { id: u.id } });
    if (!draft || draft.paperUploadId !== id) return NextResponse.json({ error: `Draft ${u.id} not found for this upload` }, { status: 404 });
    ...
    if (parsed.data.correctOptionLabel !== undefined) updateData.correctOptionLabel = parsed.data.correctOptionLabel;
    ...
    if (parsed.data.needsReview !== undefined) updateData.needsReview = parsed.data.needsReview;
    if (parsed.data.status !== undefined) updateData.status = parsed.data.status;
    const updated = await prismaClient.draftQuestion.update({ where: { id: u.id }, data: updateData });
    results.push({ ...updated, options: JSON.parse(updated.options) });
  }
