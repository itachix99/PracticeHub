...
export async function PATCH(
  ...
  const draft = await prisma.draftQuestion.findUnique({
    where: { id: questionId },
  });
  ...
  if (parsed.data.options !== undefined)
    data.options = parsed.data.options as unknown as never;
  if (parsed.data.correctOptionLabel !== undefined)
    data.correctOptionLabel = parsed.data.correctOptionLabel;
  if (parsed.data.explanation !== undefined)
    data.explanation = parsed.data.explanation;
  if (parsed.data.marks !== undefined) data.marks = parsed.data.marks;
  if (parsed.data.needsReview !== undefined)
    data.needsReview = parsed.data.needsReview;
  if (parsed.data.status !== undefined) data.status = parsed.data.status;
  const updated = await prisma.draftQuestion.update({
    where: { id: questionId },
    data,
  });
  return NextResponse.json({
