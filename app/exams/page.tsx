...
export default async function ExamsPage({
  ...
  const sort =
    (params.sort as "latest" | "oldest" | "title" | "popular" | undefined) ||
    "latest";
  const page = parseInt(params.page || "1", 10);
  const limit = parseInt(params.limit || "9", 10);

  const validSort = ["latest", "oldest", "title", "popular"].includes(sort)
    ? (sort as "latest" | "oldest" | "title" | "popular")
    : "latest";
    ...
    <div className="container mx-auto max-w-7xl px-4 py-8 sm:px-6">
      <div className="mb-6 flex items-center justify-between">
        ...
        <Badge variant="secondary">
          {total} {total === 1 ? "exam" : "exams"}
          {q || organization ? " filtered" : ""}
        </Badge>
      </div>
          ...
          <CardHeader>
            ...
            <CardDescription>
              {q || organization ? (
                ...
                <>
                  Run{" "}
                  <code className="bg-muted rounded px-1">npm run seed</code> to
                  generate the SSC mock.
                </>
              )}
            </CardDescription>
          </CardHeader>
          ...
          <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {exams.map((exam) => {
              ...
              const minutes = config.timing?.totalSec
                ...
                : 60;
              const questionCount =
                exam.currentVersion?.sections.reduce(
                  ...
                  <CardContent className="mt-auto space-y-3">
                    <div className="text-muted-foreground flex flex-wrap items-center gap-3 text-sm">
                      <span className="flex items-center gap-1">
                        <Clock className="size-4" /> {minutes} min
                      </span>
                      <span className="flex items-center gap-1">
                        <FileText className="size-4" /> {sectionCount} sections
                      </span>
                      {questionCount > 0 && (
                        <span className="flex items-center gap-1">
                          <FileText className="size-4" /> {questionCount} Qs
                        </span>
                      )}
                    </div>
                    <div className="flex gap-2">
                      <Link
                        ...
                        className="bg-primary text-primary-foreground hover:bg-primary/90 inline-flex h-9 flex-1 items-center justify-center rounded-md px-4 text-sm font-medium shadow"
                      >
                        View
                      </Link>
                      <Link
