...
export default async function ResultPage({
  ...
  const attempt = await prisma.examAttempt.findUnique({
    ...
    include: {
      result: true,
      answers: {
        include: { question: { include: { options: true, answer: true } } },
      },
      version: {
        include: {
          sections: {
            include: {
              questions: { include: { options: true, answer: true } },
            },
          },
        },
      },
  ...
  try {
    const raw = result.sectionWise as unknown;
    sectionWise =
      typeof raw === "string"
        ? JSON.parse(raw as string)
        : ((raw as typeof sectionWise) ?? []);
  } catch {}
  ...
  return (
    <div className="container mx-auto max-w-5xl px-4 py-8 sm:px-6">
      <div className="mb-6 flex items-center justify-between">
        ...
        <Badge variant={result.percentage >= 50 ? "default" : "secondary"}>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <Card>
          <CardHeader className="pb-2">
            ...
            <CardTitle className="text-2xl">
              {result.attempted} / {result.attempted + result.unattempted}
            </CardTitle>
          </CardHeader>
          <CardContent className="text-muted-foreground text-xs">
        ...
        <Card>
          <CardHeader className="pb-2">
            ...
            <CardTitle className="flex items-center gap-1 text-2xl">
              <Clock className="size-5" /> {timeTakenMin}m {timeTakenSec}s
            </CardTitle>
          </CardHeader>
          <CardContent className="text-muted-foreground text-xs">
            ...
            {sectionWise.map((s) => (
              <div key={s.sectionId} className="rounded-md border p-3">
                ...
                <p className="text-muted-foreground text-xs">
                  {s.score} / {s.max} • {s.attempted} attempted • {s.correct}{" "}
                  correct
                </p>
      ...
      <Card className="mt-6">
        <CardHeader>
          ...
          <CardDescription>
            Your response vs correct answer • marks earned/lost
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          {attempt.answers
            .sort((a, b) => {
              ...
              const orderA = (() => {
                for (const sec of attempt.version.sections) {
                  const idx = sec.questions.findIndex(
                    (q) => q.id === a.questionId
                  );
                  if (idx !== -1) return sec.order * 100 + idx;
                }
                return 999;
              })();
              const orderB = (() => {
              ...
              return (
                <div key={ans.questionId} className="rounded-md border p-4">
                  ...
                  <p className="text-muted-foreground mt-1 text-xs">
                    {q.sectionName} • {q.marks} marks, negative{" "}
                    {q.negativeMarks} • {ans.state} •{" "}
                    {marksEarned > 0 ? `+${marksEarned}` : marksEarned} earned
                  </p>
                  <div className="mt-3 grid gap-2">
                    {q.options.map((opt) => {
                      ...
                      return (
                        <div
                          key={opt.id}
                          className={`flex gap-2 rounded-md border p-2 text-sm ${isCorrectOpt ? "border-green-600 bg-green-50" : ""} ${isSelected && !isCorrectOpt ? "border-red-600 bg-red-50" : ""} ${isSelected ? "ring-1" : ""}`}
                        >
                  ...
                  <div className="mt-2 grid gap-1 text-xs">
                    ...
                    {q.explanation && (
                      <p className="bg-muted mt-1 rounded p-2">
                        <span className="font-medium">Explanation:</span>{" "}
                        {q.explanation}
                      </p>
                    )}
                    <p className="text-muted-foreground">
                      Time spent: {ans.timeSpentMs} ms
                    </p>
                  </div>
                </div>
              );
      ...
      <div className="mt-6 flex gap-2">
        <Button asChild>
          <Link href={`/exam/${slug}`}>Retake</Link>
        </Button>
        <Button variant="outline" asChild>
          <Link href="/exams">Library</Link>
        </Button>
        <Button variant="outline" asChild>
